/**
 * Espelha src/utils/calculateScore.ts para gravar finScore após sync Pluggy.
 */
function monthKey(date) {
  return `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}`;
}

function scoreSavingsRate(ratePct, hasCashflowData) {
...
function scoreEmergencyReserve(months, hasExpenseHistory) {
...
function scoreBudgetAdherence(overBudgetRatio) {
...
function scoreCreditPressure(snapshot) {
...
function scoreEntryConsistency(recentEntriesCount) {
...
function calculateFinScore(entries, goals, budgets, accountBalances, accountMeta, creditSnapshot) {
  const now = new Date();
  ...
  for (const entry of entries || []) {
    ...
    if (entry.account) {
      accountCurrentBalances[entry.account] =
        (accountCurrentBalances[entry.account] ?? 0) + (entry.type === 'receita' ? entryValue : -entryValue);
    }
  ...
  const availableBalance = Object.entries(accountCurrentBalances).reduce((sum, [account, balance]) => {
    if (accountMeta && accountMeta[account] && accountMeta[account].incluirNaSoma === false) return sum;
    return sum + balance;
  }, 0);
...
module.exports = { calculateFinScore };
