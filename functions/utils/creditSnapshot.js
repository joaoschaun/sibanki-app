/**
 * Espelha src/utils/creditSnapshot.ts para uso nas Cloud Functions (ex.: após sync Pluggy).
 */
...
function isDebtLikeLabel(value) {
...
function buildCreditSnapshot(input) {
  ...
  for (const card of activeCards) {
    ...
    const bills = Array.isArray(card.faturas) ? card.faturas : [];
  ...
  const monthlyDebtCommitment = roundCurrency(
    (input.recurrents || []).reduce((sum, recurrent) => {
      if (!recurrent || recurrent.active === false || recurrent.type !== 'despesa') return sum;
  ...
  const explicitDueSoonAmount = dueSoonOpenObligations.reduce(
    (sum, obligation) => sum + Number(obligation.amount || 0),
    0,
  );
  ...
  const totalUsed = roundCurrency(Math.max(Number(persisted?.totalUsed || 0), derivedTotalUsed, explicitUsed));
...
module.exports = { buildCreditSnapshot };
