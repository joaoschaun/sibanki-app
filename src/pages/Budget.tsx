...
import {
  ...
  ArrowRightLeft, Info, LayoutList,
} from 'lucide-react';
...
export default function Budget() {
  const { user, entries, budgets: budgetsRaw, loading, data } = useAppContext();
  ...
  async function handleDistribuir() {
    ...
    setBusy(true);
    try { await updateBudgets(user.uid, next); triggerWithToast('budget_created'); }
    finally { setBusy(false); }
  }
  ...
  async function handleMove() {
    ...
    setBusy(true);
    try { await updateBudgets(user.uid, next); setMoveModal(null); }
    finally { setBusy(false); }
  }
      ...
      {mode === 'envelope' && (
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div className="bg-si-card border border-si-border rounded-2xl p-5 text-center">
            ...
            <div className={`rounded-2xl border p-5 text-center ${zeroado ? 'bg-emerald-500/8 border-emerald-500/25' : 'bg-amber-500/8 border-amber-500/25'}`}>
          ...
          <div className="space-y-2">
            {categorias.map(cat => {
              ...
              const gasto    = gastosByCat[cat] ?? 0;
              const restante = alocado - gasto;
              const pct      = alocado > 0 ? Math.min(100, (gasto / alocado) * 100) : (gasto > 0 ? 100 : 0);
              const over     = gasto > alocado && alocado > 0;
              return (
