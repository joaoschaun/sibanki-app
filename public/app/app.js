...
gBtns.forEach(function(b){b.disabled=true;b.innerHTML='<span class="spinner"></span> Redirecionando...'});
...
gBtns.forEach(function(b){b.innerHTML='<span class="spinner"></span> Autenticando...'});
...
toast('Use Entrar com e-mail ou permita popups para este site.','err');
gBtns.forEach(function(b){b.disabled=false;b.innerHTML='Entrar com Google'});
return;
}
if(err&&err.code==='auth/cancelled-popup-request'){gBtns.forEach(function(b){b.disabled=false;b.innerHTML='Entrar com Google'});return;}
toast('Erro: '+err.message,'err');
gBtns.forEach(function(b){b.disabled=false;b.innerHTML='Entrar com Google'});
});
...
function sibCadDesistir(){sibCadHide();if(sibCadFromOnb){sibOnbShowScreen('welcome');}else{toast('Cadastro pode ser completado depois em Perfil.','info');}}
...
function sibCadFinishAndGo(){sibCadHide();if(sibCadFromOnb){sibOnbShowConfig();}else{go('contas',null);toast('Cadastro salvo! Adicione sua primeira conta.','ok');}}
...
}catch(e){console.error('applyDocFromServer error:',e);toast('Erro ao preparar a tela. Recarregue a página.','err');}
...
toast('Carregamento demorou. Exibindo tela com dados locais.','info');
...
toast('Falha ao salvar. Seus dados podem ser perdidos!','err');
});
},3000);
});
}
...
if(!U||!U.uid)return;
if(!confirm('Tem certeza? Todos os lançamentos, metas, contas, investimentos e categorias serão apagados. Sua conta de login continua. Esta ação não pode ser desfeita.'))return;
if(!confirm('Última confirmação: realmente apagar TUDO e recomeçar do zero?'))return;
var emptyData={
entries:[],investments:[],achievements:{},goals:[],budgets:{},orcamentosByMonth:{},
...
if(typeof toast==='function')toast('Dados apagados. Recomeçando do zero.','ok');
initUI();
setTimeout(function(){if(typeof go==='function')go('dash',null);},50);
...
if(typeof toast==='function')toast('Erro ao apagar dados: '+e.message,'err');
});
}
...
try{await db.collection('users').doc(U.uid).update({dashboardLayout:widgets,layoutAtualizadoEm:firebase.firestore.FieldValue.serverTimestamp()});toast('Layout salvo!','ok');}catch(e){toast('Erro ao salvar: '+e.message,'err');}
}
...
closeEd();saveData();renderAll();toast('Atualizado!','ok');
}
...
var val=pf('tfV');
if(!date){toast('Selecione a data','err');return}
if(!from||!to||from===to){toast('Selecione contas diferentes','err');return}
if(!val||val<=0){toast('Valor inválido','err');return}
...
saveData();renderAll();toast('Transferencia de R$ '+val.toFixed(2)+' realizada!','ok');
...
if(!de){toast('Digite descrição','err');return}
if(!val||val<=0){toast('Valor inválido','err');return}
if(!day||day<1||day>31){toast('Dia inválido','err');return}
...
saveData();rnRc();toast('Recorrente adicionado!','ok');
...
if(changed){saveData();toast('Custos fixos do mês lançados!','ok')}
}
function popRcSels(){
...
if(typeof toast==='function')toast('Tour concluído! Você ganhou +50 XP 🎉','ok');
}).catch(function(e){console.error('tourCompleto save',e);});
}else{if(typeof toast==='function')toast('Tour concluído! Você ganhou +50 XP 🎉','ok');}
}
_sibTourState=null;
}
...
if(ctaAgora){ctaAgora.addEventListener('click',function(){if(_sibTourState&&_sibTourState.onEnd)_sibTourState.onEnd();sibTourEnd();if(typeof openInvestorProfilePopup==='function')openInvestorProfilePopup();if(typeof toast==='function')toast('Responda ao questionário para descobrir seu perfil.','ok');});}
if(ctaDepois){ctaDepois.addEventListener('click',function(){if(_sibTourState&&_sibTourState.onEnd)_sibTourState.onEnd();sibTourEnd();if(typeof toast==='function')toast('Você pode fazer o questionário depois na aba Perfil.','ok');});}
});
...
if(!q){toast('Digite uma pergunta','err');return;}
addIAMsg('user',q);
...
toast('IA já está integrada! Nenhuma chave necessária.','ok');
}
...
try{rKPI();}catch(x){}
}catch(err){console.error('saveCashMode error',err);toast('Não foi possível salvar preferência do modo caixa','err');}
}
...
saveData();renderAll();toast('Cartão removido','ok');
}
...
if(!desc){toast('Preencha a descrição da compra','err');return;}
if(!fatValEl||!fatValEl.value){toast('Preencha o valor da compra','err');return;}
var total=pf('fatVal');
...
if(!desc){toast('Digite uma descrição','err');return}
if(!total||total<=0){toast('Digite o valor','err');return}
if(!date){toast('Selecione a data','err');return}
...
'<div class="kpi kpi-saldo-wrap" onclick="openKpiModal(\'saldo\')"><div class="kpi-saldo-left"><div class="kl"><i data-lucide="wallet" style="width:16px;height:16px;stroke:currentColor;stroke-width:2;vertical-align:middle;margin-right:6px"></i>Saldo do M\u00eas</div><div class="kv '+saldoCls+'">'+fmt(salMes)+'</div><div class="ks '+(salMes>=0?'positive':'negative')+'">Geral: '+fmt(sal)+' \u25b8</div></div><canvas id="sparkline-saldo" width="80" height="40"></canvas></div>'+
'<div class="kpi" onclick="showPatrimonio()"><div class="kl"><i data-lucide="trending-up" style="width:16px;height:16px;stroke:currentColor;stroke-width:2;vertical-align:middle;margin-right:6px"></i>Patrim\u00f4nio Total</div><div class="kv">'+fmt(patrimônio)+'</div><div class="ks">Contas + Invest. \u25b8</div></div>'+
'</div>'+
'<div class="kpi-grid">'+
'<div class="kpi kpi-receitas" onclick="openKpiModal(\'receitas\')"><div class="kl"><i data-lucide="wallet" style="width:16px;height:16px;stroke:currentColor;stroke-width:2;vertical-align:middle;margin-right:6px"></i>Receitas do M\u00eas</div><div class="kv">'+fmt(recMes)+'</div><div class="ks">Total: '+fmt(tr)+' \u25b8</div></div>'+
'<div class="kpi kpi-despesas" onclick="openKpiModal(\'despesas\')"><div class="kl"><i data-lucide="receipt" style="width:16px;height:16px;stroke:currentColor;stroke-width:2;vertical-align:middle;margin-right:6px"></i>Despesas do M\u00eas</div><div class="kv">'+fmt(despMes)+'</div><div class="ks '+(pendMes>0?'negative':'')+'">'+(pendMes>0?'Pendente: '+fmt(pendMes):'Total: '+fmt(td))+' \u25b8</div></div>'+
'<div class="kpi kpi-custos" onclick="openKpiModal(\'custos\')"><div class="kl"><i data-lucide="pin" style="width:16px;height:16px;stroke:currentColor;stroke-width:2;vertical-align:middle;margin-right:6px"></i>Custos Fixos</div><div class="kv">'+fmt(custoFixo)+'</div><div class="ks">Rec. fixa: '+fmt(recFixa)+' \u25b8</div></div>'+
'<div class="kpi kpi-investido" onclick="openKpiModal(\'investido\')"><div class="kl"><i data-lucide="trending-up" style="width:16px;height:16px;stroke:currentColor;stroke-width:2;vertical-align:middle;margin-right:6px"></i>Investido</div><div class="kv">'+fmt(totalInvAtual)+'</div><div class="ks">Detalhes \u25b8</div></div>'+
'</div>';
